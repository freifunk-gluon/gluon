...
static void ebt_ip_call(char *mod, struct in_addr ip)
{
	...
	snprintf(str, sizeof(str),
			NFT " %s element bridge gluon datips { %s }",
			mod, inet_ntoa(ip));
...
static void ebt_mac_call(char *mod, struct mac_addr *mac)
{
	...
	snprintf(str, sizeof(str),
			NFT " %s element bridge gluon limitmac { %s }",
			mod, mac_ntoa(mac));
...
{
	int ret = system(NFT " flush set bridge gluon datips");

	if (ret)
		fprintf(stderr, "Error flushing datips set\n");
}

static void ebt_tl_flush(void)
{
	int ret = system(NFT " flush set bridge gluon limitmac");

	if (ret)
		fprintf(stderr, "Error flushing limitmac set\n");
}
