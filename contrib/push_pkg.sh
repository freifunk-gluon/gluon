		...
		if [ -n "$filename" ]; then
			...
			ssh -p "${ssh_port}" "root@${ssh_host}" "
				...
				if [ '$preserve_config' -eq 1 ]; then
					echo 'Doing backup of /etc/config'
					rm -rf /etc/config.bak/
					mkdir /etc/config.bak/
					cp -r /etc/config/* /etc/config.bak/
				fi
				echo Running opkg:
				opkg install --force-reinstall '/tmp/${filename}'
				echo Running opkg finished: ok.
				if [ '$preserve_config' -eq 1 ]; then
					echo 'Restoring /etc/config'
					for f in \$(find /etc/config.bak/ -mindepth 1); do
						echo \"Restoring /etc/config/\$(basename \"\$f\")\"
						mv -f \"\$f\" /etc/config/
					done
					rmdir /etc/config.bak
				fi
				rm '/tmp/${filename}'
				gluon-reconfigure
			"
