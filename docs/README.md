...
1. Go to docs folder `cd docs` and create an environment using `python -m venv _venv`
2. Activate the environment using `. ./_venv/bin/activate`
3. Install the requirements using `pip install -r requirements.txt`
4. Build the docs using `make html`
5. Serve the result on port 8000 using `python -m http.server -b 127.0.0.1 -d _build/html`
