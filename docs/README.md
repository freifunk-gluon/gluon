...
0. Go to repository root and create environment using `python -m venv venv`
2. Activate the environment using `. ./venv/bin/activate`
3. Install the requirements using `pip install -r docs/requirements.txt`
4. Build the docs using `make -C docs html`
5. Serve the result on port 8000 using `python -m http.server -b 127.0.0.1 -d docs/_build/html`
